      ...
      {activeSection === 'agents' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {AGENTS_CATALOG.map((ag) => (
            <div key={ag.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-md space-y-3 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-2 text-xs">
                  <div>
                    <span className="text-slate-400 font-semibold block text-[11px]">Activación:</span>
                    <span className="text-slate-300 font-mono text-[11px]">{ag.activates}</span>
                  </div>

                  <div>
                    <span className="text-slate-400 font-semibold block text-[11px]">Artefactos Salida:</span>
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {ag.outputs.map((out, idx) => (
                        <span key={idx} className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-slate-950 border border-slate-800 text-sky-300">
                          {out}
                        </span>
                      ))}
                    </div>
              ...
              <div className="border-t border-slate-800 pt-3 text-[11px] text-slate-400 space-y-1">
                ...
                <div className="text-rose-400/90"><strong>Escalar Si:</strong> {ag.mustEscalate.join(', ')}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeSection === 'skills' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {SKILLS_CATALOG.map((sk) => (
            <div key={sk.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-md space-y-3">
              ...
              <div className="text-[11px] font-mono text-slate-500 bg-slate-950 p-2 rounded border border-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs font-mono">
            {[
              ...
              <div key={i} className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
                <div className="font-bold text-sky-400">{cnt.name}</div>
                <div className="text-[11px] text-slate-400 font-sans">{cnt.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-md space-y-4">
          ...
          <div className="space-y-3 text-xs text-slate-300 leading-relaxed">
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
              <strong className="text-amber-400 block font-sans font-bold">1. Sin Invención de Valores o Criterios:</strong>
              <p>No inventar valores, fechas, evidencia, criterios, configuraciones, fuentes normativas ni conclusiones.</p>
            </div>

            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
              <strong className="text-amber-400 block font-sans font-bold">2. Trazabilidad Estricta:</strong>
              <p>Conservar la trazabilidad desde la evidencia hasta el finding, criterio, recomendación y entregable.</p>
            </div>

            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
              <strong className="text-amber-400 block font-sans font-bold">3. Presupuesto de Correcciones (3 Ciclos Max):</strong>
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
              <strong className="text-amber-400 block font-sans font-bold">4. Estado "NO EMITIR" Obligatorio:</strong>
