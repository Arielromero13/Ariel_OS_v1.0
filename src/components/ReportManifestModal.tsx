      ...
      <div className="bg-ink-900 border border-ink-700 rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === 'document' ? (
            ...
            <div className="bg-ink-950 border border-ink-700 rounded-xl p-8 space-y-6 text-ink-100 font-sans shadow-inner">
                  ...
                  <tbody className="divide-y divide-slate-800 text-ink-200">
                    {workItem.measurements.map((m) => (
                      <tr key={m.id} className="hover:bg-ink-900/40 font-mono">
                        <td className="p-2.5 border border-ink-700 font-bold text-signal-300">{m.point_code}</td>
                        <td className="p-2.5 border border-ink-700 font-sans">{m.location_description}</td>
                        <td className="p-2.5 border border-ink-700 text-center font-bold">{m.measured_resistance_ohm} Ω</td>
              ...
              {workItem.findings.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-xs font-bold text-fail-400 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <div className="space-y-2">
                    {workItem.findings.map((f) => (
                      <div key={f.id} className="bg-fail-950/20 border border-fail-900/60 p-3 rounded-lg text-xs space-y-1">
                        <div className="font-bold text-fail-300">{f.title}</div>
              ...
              <div className="pt-6 border-t border-ink-700 grid grid-cols-2 gap-8 text-center text-xs">
                <div className="border-t border-ink-600 pt-2 text-ink-300">
                  <span className="block font-bold text-ink-100">Revisión Técnica & Visual</span>
                  <span className="text-[11px]">Ariel OS Agent Framework</span>
                </div>
                <div className="border-t border-ink-600 pt-2 text-ink-300">
                  <span className="block font-bold text-ink-100">Aprobación de Emisión Humana</span>
            ...
            <div className="space-y-3">
              ...
              <pre className="bg-ink-950 p-4 rounded-xl border border-ink-700 text-xs font-mono text-signal-300 overflow-x-auto max-h-[480px]">
